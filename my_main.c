...
void first_pass(int *num_frames, char *name) {
  ...
  for (i=0;i<lastop;i++) {
	...
	*num_frames = (int)op[i].op.frames.num_frames;
	...
	printf("FRAMES value must be set once\n");
	exit(42);
      }
      break;
	...
	printf("BASENAME must be set once and only once\n");
	exit(42);
      }
      break;
	...
	printf("VARY must have positive frames\n");
	exit(42);
      }
      if (op[i].op.vary.end_frame < 0){
	printf("VARY must not exceed last frame\n");
	exit(42);
      }
      if (op[i].op.vary.start_frame > op[i].op.vary.end_frame){
	printf("VARY first frame must be before final frame\n");
	exit(42);
      }
      break;
    }
  ...
  if (vary_frame && !frame_frame){
    printf("VARY must have TOTAL frames set\n");
    exit(42);
  }
  if (!name_frame && (vary_frame || frame_frame)){
    printf("No BASENAME given. Defaulting to unicorn\n");
    strcpy(name, "unicorn.png");
  }
}
...
struct vary_node ** second_pass(int frames) {
  ...
  for (i = 0; i < lastop; i++) {
    if (op[i].opcode == VARY){
      int frame_number = op[i].op.vary.end_frame-op[i].op.vary.start_frame;
      double start = op[i].op.vary.start_val;
      double inc = (op[i].op.vary.end_val-start)/frame_number;
